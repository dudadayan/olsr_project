...
OLSR::initialize(int stage)
{
    if (stage==4)
    {
        ...
        tc_timer_.resched(tc_ival_);
        mid_timer_.resched(mid_ival_);
        if (use_mac())
        {
            linkLayerFeeback();
        }
...
OLSR::link_sensing(OLSR_msg& msg, const nsaddr_t &receiver_iface, const nsaddr_t &sender_iface,const int &index)
{
    ...
    for (int i = 0; i < hello.count; i++)
    {
        ...
        for (int j = 0; j < hello_msg.count; j++)
        {
            ...
            /// NEW NEW NEW
            /// Here we need to change the code according to our
            /// proposal
            ///
            //if (hello_msg.nb_iface_addr(j) == receiver_iface)
            if (hello_msg.nb_node(j).nb_iface_addr() == receiver_iface)
            {
            ...
            if (link_tuple->sym_time() >= now)
            {
                ...
                for (int i = 0; i < hello.count; i++)
                {
                    ...
                    for (int j = 0; j < hello_msg.count; j++)
                    {
                        ...
                        nsaddr_t nb2hop_addr = hello_msg.nb_node(j).nb_iface_addr();
